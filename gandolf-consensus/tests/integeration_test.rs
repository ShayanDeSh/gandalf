...
async fn client_read_requset(count: u32, addr: String, sleep_duration: Duration) -> gandolf_consensus::Result<()> {
    sleep(sleep_duration).await;
    let mut con = client::connect(addr).await?;
    for i in 0..count {
        let key = format!("foo{}", i);
...
#[tokio::test(flavor = "multi_thread", worker_threads = 5)]
async fn test_direct_read() -> gandolf_consensus::Result<()> {
    let cluster = kvs_cluster_of_nth(5).await?;
...
#[tokio::test(flavor = "multi_thread", worker_threads = 5)]
async fn test_indirect_read() -> gandolf_consensus::Result<()> {
    let cluster = kvs_cluster_of_nth(5).await?;
